...
func (r *tierRepo) GetByID(ctx context.Context, id uuid.UUID) (*models.Tier, error) {
	row := r.db.QueryRow(ctx, `
		SELECT id, name, debt_ceiling, commission_rate, commission_cap, created_at, updated_at
		FROM tiers WHERE id = $1`, id)

	t := &models.Tier{}
	err := row.Scan(&t.ID, &t.Name, &t.DebtCeiling, &t.CommissionRate, &t.CommissionCap, &t.CreatedAt, &t.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return t, nil
...
func (r *tierRepo) List(ctx context.Context) ([]models.Tier, error) {
	...
	for rows.Next() {
		var t models.Tier
		if err := rows.Scan(&t.ID, &t.Name, &t.DebtCeiling, &t.CommissionRate, &t.CommissionCap, &t.CreatedAt, &t.UpdatedAt); err != nil {
			return nil, err
		}
