...
type ShipmentRepository interface {
	...
	UpsertDispatch(ctx context.Context, s *models.Shipment) error
	GetByOrderID(ctx context.Context, tenantID, orderID uuid.UUID) (*models.Shipment, error)
	GetByCarrierRef(ctx context.Context, carrierRef string) (*models.Shipment, error)
...
func (r *shipmentRepo) UpsertDispatch(ctx context.Context, s *models.Shipment) error {
	return r.db.QueryRow(ctx, `
		INSERT INTO shipments (order_id, tenant_id, status, carrier_ref, tracking_number, carrier_history)
		VALUES ($1, $2, $3, $4, $5, $6)
		...
		DO UPDATE SET
			...
			carrier_history = EXCLUDED.carrier_history,
			updated_at = NOW()
		RETURNING id, status, created_at, updated_at`,
		s.OrderID, s.TenantID, s.Status, s.CarrierRef, s.TrackingNumber, s.CarrierHistory,
	).Scan(&s.ID, &s.Status, &s.CreatedAt, &s.UpdatedAt)
}
...
func (r *shipmentRepo) GetByCarrierRef(ctx context.Context, carrierRef string) (*models.Shipment, error) {
	s := &models.Shipment{}
	err := r.db.QueryRow(ctx, `
		SELECT id, order_id, tenant_id, status, carrier_ref, tracking_number, carrier_history, created_at, updated_at
		FROM shipments WHERE carrier_ref = $1`, carrierRef,
	).Scan(&s.ID, &s.OrderID, &s.TenantID, &s.Status, &s.CarrierRef,
		&s.TrackingNumber, &s.CarrierHistory, &s.CreatedAt, &s.UpdatedAt)
	if err != nil {
		return nil, err
...
func (r *shipmentRepo) AppendCarrierEvent(ctx context.Context, tenantID, id uuid.UUID, event []byte) error {
	tag, err := r.db.Exec(ctx, `
		UPDATE shipments
		SET carrier_history = CASE
				WHEN carrier_history IS NULL OR carrier_history = 'null'::jsonb THEN jsonb_build_array($1::jsonb)
				WHEN jsonb_typeof(carrier_history) = 'array' THEN carrier_history || jsonb_build_array($1::jsonb)
				ELSE jsonb_build_array(carrier_history, $1::jsonb)
			END,
			updated_at = NOW()
